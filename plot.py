...
def update_readme_table(df, readme_path="README.md"):
    ...
    for i, row in df.iterrows():
        ...
        pct = (
            0.0
            if i == 0
            else ((row["pointer_chasing_ns"] - l1_latency) / l1_latency) * 100
        )
...
def create_comparison_plot(df):
    ...
    fig.add_trace(
        go.Bar(
            x=df["cache_level"],
            y=alloc_vals,
            name=f"Allocated ({alloc_unit})",
            marker_color="rgba(50, 50, 200, 0.5)",
            yaxis="y2",
            opacity=0.5,
            text=alloc_text,
            hoverinfo="text+name",
    ...
    fig.update_layout(
        ...
        xaxis=dict(type="category", gridcolor="rgba(200,200,200,0.2)"),
        hovermode="x unified",
        plot_bgcolor="white",
        height=700,
        width=1000,
    ...
    ax2 = ax1.twinx()
    ax1.plot(
        df["cache_level"], df["pointer_chasing_ns"], "o-r", label="Pointer Chasing"
    )
